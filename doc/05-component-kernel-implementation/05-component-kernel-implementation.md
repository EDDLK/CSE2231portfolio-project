...
It's really convenient to use a nested map as the internal representation. All keys and nested keys in the map must be valid currency codes, and every stored exchange rate must be greater than 0. The map itself, along with all nested maps, must be non-null. The correspondence is straightforward: for example, exchangeRates.get("USD").get("CNY") = x means that 1 USD equals x CNY. Conceptually, this structure models a directed graph where each node represents a currency and the edges represent exchange rates between them.
...
- Designed kernel implementation for CurrencyConverter component
